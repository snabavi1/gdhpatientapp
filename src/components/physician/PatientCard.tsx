...
const PatientCard: React.FC<PatientCardProps> = ({ patient, darkMode }) => {
  ...
  return (
    <div className={`group p-3 sm:p-4 lg:p-5 rounded-xl border transition-all duration-300 hover:shadow-lg hover:scale-[1.01] overflow-hidden ${
      isLongerWait 
        ? darkMode 
      ...
      <div className="flex flex-col space-y-3 xl:hidden">
        {/* Row 1: Acuity Badge and Wait Time */}
        ...
        {/* Row 2: Patient Name and Room */}
        <div className="min-w-0">
          <div className={`font-bold text-sm sm:text-base leading-tight ${darkMode ? 'text-white' : 'text-gray-900'} break-words`}>
            {patient.entry} {patient.name}, {patient.age}{patient.gender}
          </div>
          <div className={`text-xs sm:text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-600'} mt-1`}>
            Room #{patient.room}
          </div>
        </div>

        {/* Row 3: Complaint */}
        <div className={`text-sm leading-relaxed ${darkMode ? 'text-gray-200' : 'text-gray-800'} break-words hyphens-auto`}>
          {patient.complaint}
        </div>

        {/* Row 4: Status and Tests */}
        <div className="space-y-2">
          <div className={`text-sm font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-800'} break-words`}>
            {patient.status}
          </div>
          <div className="flex flex-col gap-1">
            {patient.test && (
              <div className={`text-xs ${darkMode ? 'text-blue-300' : 'text-blue-600'} font-medium break-words`}>
                🧪 {patient.test}
              </div>
            )}
            {patient.results && (
              <div className="text-xs text-emerald-600 font-medium break-words">
                ✅ {patient.results}
              </div>
            )}
            {isLongerWait && (
        ...
        <div className="flex flex-col xs:flex-row gap-2 pt-2">
          <Button 
            ...
            className="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-semibold px-3 py-2 transition-all duration-200 shadow-md hover:shadow-lg text-xs w-full xs:w-auto"
          >
          ...
          <Button 
            ...
            className="bg-gradient-to-r from-sky-600 to-sky-700 hover:from-sky-700 hover:to-sky-800 text-white font-semibold px-3 py-2 transition-all duration-200 shadow-md hover:shadow-lg text-xs w-full xs:w-auto"
          >
      ...
      <div className="hidden xl:grid xl:grid-cols-12 xl:gap-4 xl:items-center">
        {/* Column 1-2: Acuity & Patient Info */}
        <div className="xl:col-span-4 flex items-center gap-3 min-w-0">
          <Badge 
            className={`${getAcuityColor()} text-white text-sm font-semibold px-3 py-1.5 transition-colors duration-200 shadow-sm flex-shrink-0`}
          >
            {getAcuityLabel()}
          </Badge>
          <div className="min-w-0 flex-1">
            <div className={`font-bold text-base leading-tight ${darkMode ? 'text-white' : 'text-gray-900'} truncate`}>
              {patient.entry} {patient.name}, {patient.age}{patient.gender}
            </div>
        ...
        {/* Column 3-6: Complaint */}
        <div className="xl:col-span-4 min-w-0">
          <div className={`text-sm font-medium leading-relaxed ${darkMode ? 'text-gray-200' : 'text-gray-800'} line-clamp-2`}>
            {patient.complaint}
          </div>
        </div>
        ...
        <div className="xl:col-span-2 min-w-0">
          <div className={`text-sm font-semibold mb-1 ${darkMode ? 'text-gray-200' : 'text-gray-800'} truncate`}>
            {patient.status}
          </div>
        ...
        {/* Column 9: Wait Time */}
        <div className="xl:col-span-1 text-center min-w-0">
          <Badge 
            variant="outline" 
            className={`${getWaitTimeColor()} border-current text-sm px-2 py-1.5 font-semibold bg-white/80 whitespace-nowrap`}
          >
        ...
        <div className="xl:col-span-1 flex gap-2 justify-end">
          <Button 
            size="sm"
            onClick={handleStartVideoCall}
            className="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-semibold px-3 py-2 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 whitespace-nowrap"
          >
          ...
          <Button 
            size="sm"
            onClick={handleRespond}
            className="bg-gradient-to-r from-sky-600 to-sky-700 hover:from-sky-700 hover:to-sky-800 text-white font-semibold px-3 py-2 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 whitespace-nowrap"
          >
            {patient.entryMethod === 'phone' ? (
